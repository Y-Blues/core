    ...
    try:
        if "/" in a_file and not a_file.split("/")[-1].startswith("test_"):
            with open(a_file, "r") as f:
                ...
                if "pelix" not in a_module_name and \
                    ...
                    if framework.app_name is None:
                        ...
                        for w_layer_pattern in w_layer_patterns:
                            w_layer_pattern = "@Layer\(name=\"" + w_layer_pattern.replace("*", ".*")
                            w_layer_pattern_2 = "@Layer\(name='" + w_layer_pattern.replace("*", ".*")
                            if re.search(w_layer_pattern, content) or re.search(w_layer_pattern_2, content):
                                bundle_loaded.append(a_module_name)
                                a_context.install_bundle(a_module_name).start()
                                return None
                ...
                if a_module_name == "ycappuccino_storage.ycappuccino_core.models.decoratrs" or a_module_name == "ycappuccino_storage.ycappuccino_core.models.utils" or a_module_name == "ycappuccino_storage.ycappuccino_core.decorator_app":
                ...
                if "@Item" in content:
                    ...
                    if framework.app_name is None:
                        ...
                        w_app_pattern_applyed = "@App\(name=\"" + framework.app_name.replace("*", ".*")
                        w_app_pattern_applyed_2 = "@App\(name='" + framework.app_name.replace("*", ".*")
        ...
        if "/" in w_file and not w_file.split("/")[-1].startswith("test_"):
            ...
            if os.path.exists(w_file) and \
                ...
                if os.path.isdir(w_file) and os.path.isfile(w_file+"/__init__.py"):
                    if a_module_name == "":
                        w_module_name = w_file.split(os.sep)[-1]
                    else:
