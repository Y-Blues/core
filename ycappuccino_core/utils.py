    ...
    try:
        if "/" in a_file and not a_file.split("/")[-1].startswith("test_"):
            ...
            with open(a_file, "r") as f:
                content = f.read()
                if a_module_name not in bundle_loaded:
                    ...
                    if "pelix" not in a_module_name and \
                        ...
                        if framework.app_layers is None:
                            a_context.install_bundle(a_module_name).start()
                        else:
                            # load layer regarding app layer to load
                            w_layer_patterns = framework.app_layers
                            for w_layer_pattern in w_layer_patterns:
                                w_layer_pattern_1 = "@Layer\(name=\"" + w_layer_pattern.replace("*", ".*")+"\"\)"
                                w_layer_pattern_2 = "@Layer\(name='" + w_layer_pattern.replace("*", ".*")+"\"\)"
                                if re.search(w_layer_pattern_1, content) or re.search(w_layer_pattern_2, content):
                                    print("{} : load {}".format(time.time(), a_module_name))
                                    a_context.install_bundle(a_module_name).start()
                                    return None

                            if "@Layer(name=" not in content or "ycappuccino_storage/ycappuccino_core/utils" in a_file:
                                a_context.install_bundle(a_module_name).start()
                                return None


                    if a_module_name == "ycappuccino_core.models.decorators" or a_module_name == "ycappuccino_core.models.utils" or a_module_name == "ycappuccino_core.decorator_app":
                        add_bundle_model(a_module_name, a_file)
                    if "@Item" in content:
                        # load module define for an app
                        if framework.app_name is None:
                            return add_bundle_model(a_module_name, a_file)
                        else:
                            for app_name in framework.app_name:
                        ...
                        if "@App(name=" not in content or "ycappuccino_storage/ycappuccino_core/utils" in a_file:
                            return add_bundle_model(a_module_name, a_file)
...
def find_and_install_bundle(a_root, a_module_name, a_context):
    ...
    w_list_model = []
    prefix_ok = False
    for prefix in  framework.bundle_prefix:
    ...
    def set_context(self,a_context):
        ...
        for w_bundle in self._init_bundles:
            if ".api" in w_bundle["module"]:
                ...
                find_and_install_bundle(w_father_path, w_father_module,  self._context)
            else:
            ...
            if self._context:
                if fullname not in bundle_loaded:
                    if ".api" in w_fullpath:
                        ...
                        find_and_install_bundle(w_father_fullpath,"", self._context)
                    else:
